...
export async function authUser(data) {
  try {
    const response = await fetch(`http://localhost/api/db/user/auth`, {
      method: "GET",
      body: data,
    });
...
export async function getUser(token) {
  try {
    const response = await fetch(`http://localhost/api/db/user/me`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      return new Error(answer.message || "Ошибка 1");
    }

    return answer || new Error("Ошибка");
  } catch (error) {
    return new Error(error.message || "Ошибка запроса");
  }
}
